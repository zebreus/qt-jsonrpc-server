...
TEST(exceptionsTests, wrongArgumentCountInheritsFromInvalidSignature) {
    ASSERT_THROW({
        throw exceptions::WrongArgumentCount(1,1);
    }, exceptions::InvalidSignature);
}

TEST(exceptionsTests, WrongArgumentTypeInheritsFromInvalidSignature) {
    ASSERT_THROW({
        throw exceptions::WrongArgumentType("a",QString("b"));
    }, exceptions::InvalidSignature);
}
