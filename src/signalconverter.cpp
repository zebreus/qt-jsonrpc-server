...
PrivateSignalConverter::PrivateSignalConverter(QObject* parent)
    : QObject(parent) {}

SignalConverter::SignalConverter(QObject* parent)
    : PrivateSignalConverter(parent), dynamicSlotOffset(100) {}
...
int SignalConverter::qt_metacall(QMetaObject::Call call,
                                 int id,
                                 void** arguments) {
...
void SignalConverter::connectIfPossible(QObject* target,
                                        const QMetaMethod& method) {
  if (method.access() == QMetaMethod::Public &&
      method.methodType() == QMetaMethod::Signal) {
    int myMethodId = method.methodIndex() + dynamicSlotOffset;
    associatedStrings[myMethodId] = method.name();
    QMetaObject::connect(target, method.methodIndex(), this,
                         myMethodId + metaObject()->methodOffset());
  }
}
